//----------------------------------------------------------------------------
// EQ6_3KSlider_MainWnd.cpp : イコライザ ( 6.3KHz ) 設定用スライダの管理を行う
//----------------------------------------------------------------------------
#include <windows.h>
#include "../App.h"
#include "MainWnd.h"
...
// 作成
//----------------------------------------------------------------------------
BOOL CEQ6_3KSlider_MainWnd::Create()
{
...
// 高さを得る
//----------------------------------------------------------------------------
int CEQ6_3KSlider_MainWnd::GetHeight() const
{
	...
	else return m_rMainWnd.GetEQ5KSlider().GetHeight();
}
//----------------------------------------------------------------------------
// 縦位置を得る
//----------------------------------------------------------------------------
int CEQ6_3KSlider_MainWnd::GetTop() const
{
	if(!m_rMainWnd.GetMenu().IsItemChecked(ID_EQ))
		return m_rMainWnd.GetPanSlider().GetTop();
	else if(m_rMainWnd.GetMenu().IsItemChecked(ID_EQ6_3K))
		return CSliderCtrl::GetTop();
	else return m_rMainWnd.GetEQ5KSlider().GetTop();
}
//----------------------------------------------------------------------------
// 位置の再設定
//----------------------------------------------------------------------------
void CEQ6_3KSlider_MainWnd::ResetPos()
{
	POINT pt;
	pt.x = m_rMainWnd.GetEQ6_3KLabel().GetLeft();
	pt.y = m_rMainWnd.GetEQ6_3KLabel().GetTop();
	ScreenToClient(m_rMainWnd, &pt);
	int nLeft = pt.x + m_rMainWnd.GetEQ6_3KLabel().GetWidth()
...
// サイズの再設定
//----------------------------------------------------------------------------
void CEQ6_3KSlider_MainWnd::ResetSize()
{
	POINT pt;
	pt.x = GetLeft();
	ScreenToClient(m_rMainWnd, &pt);
...
// コマンド
//----------------------------------------------------------------------------
void CEQ6_3KSlider_MainWnd::OnCommand(int id, HWND hwndCtl, UINT codeNotify)
{
...
// スクロールされた
//----------------------------------------------------------------------------
void CEQ6_3KSlider_MainWnd::OnHScroll(HWND hwndCtl, UINT code, int pos)
{
	LONG trackpos = GetThumbPos();
	m_rMainWnd.SetEQ6_3K((int)trackpos);
	m_rMainWnd.GetEQ6_3KLabel().SetEQ6_3K(trackpos);
}
//----------------------------------------------------------------------------
// キーボードが押された
//----------------------------------------------------------------------------
void CEQ6_3KSlider_MainWnd::OnKeyDown(UINT vk, int cRepeat, UINT flags)
{
	if(vk == VK_TAB) {
		...
		SetThumbPos(0);
		m_rMainWnd.SetEQ6_3K(0);
		m_rMainWnd.GetEQ6_3KLabel().SetEQ6_3K(0);
		return;
	}
...
// クリックされた
//----------------------------------------------------------------------------
void CEQ6_3KSlider_MainWnd::OnLButtonDown(int x, int y, UINT keyFlags)
{
	RECT rc;
...
// マウスホイールがスクロールされた
//----------------------------------------------------------------------------
BOOL CEQ6_3KSlider_MainWnd::OnMouseWheel(UINT nFlags, int zDelta, POINTS pt)
{
	tstring strEQ = m_rMainWnd.GetEQ6_3KLabel().GetEdit().GetText();
	...
	m_rMainWnd.GetEQ6_3KLabel().SetEQ6_3K(n);
	return FALSE;
}
//----------------------------------------------------------------------------
// 右クリックが離された
//----------------------------------------------------------------------------
void CEQ6_3KSlider_MainWnd::OnRButtonUp(int x, int y, UINT keyFlags)
{
	m_rClickMenu.Create();
	POINT pt;
	GetCursorPos(&pt);
