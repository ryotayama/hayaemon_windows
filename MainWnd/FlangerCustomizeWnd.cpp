...
// コンストラクタ
//----------------------------------------------------------------------------
CFlangerCustomizeWnd::CFlangerCustomizeWnd(CApp & app, CMainWnd & mainWnd)
...
// デストラクタ
//----------------------------------------------------------------------------
CFlangerCustomizeWnd::~CFlangerCustomizeWnd()
{
}
//----------------------------------------------------------------------------
// 作成
//----------------------------------------------------------------------------
BOOL CFlangerCustomizeWnd::Create()
{
	Destroy();

	RegisterClass(_T("FlangerCustomizeWndCls"));
	m_hWnd = CreateWindowEx(WS_EX_TOOLWINDOW, _T("FlangerCustomizeWndCls"),
#if JP
							_T("フランジャーのカスタマイズ"),
#else // JP
							_T("Customize Flanger"),
#endif // JP
...
// 次のコントロールにフォーカス
//----------------------------------------------------------------------------
void CFlangerCustomizeWnd::SetNextFocus()
{
	switch(m_nFocus)
	{
		case 0:
...
// 前のコントロールにフォーカス
//----------------------------------------------------------------------------
void CFlangerCustomizeWnd::SetPreviousFocus()
{
	switch(m_nFocus)
	{
		case 0:
...
// コマンドが送られた
//----------------------------------------------------------------------------
void CFlangerCustomizeWnd::OnCommand(int id, HWND hwndCtl, UINT codeNotify)
{
		...
		hwndCtl == m_waveformEdit || hwndCtl == m_delayEdit)
	{
		...
		if(codeNotify == EN_SETFOCUS) {
			...
			if(hwndCtl == m_waveformEdit) _stprintf_s(buf, _T("%d"), (int)n);
...
// キャンセルボタンが押された
//----------------------------------------------------------------------------
void CFlangerCustomizeWnd::OnCancelButtonClicked()
{
	Close();
}
//----------------------------------------------------------------------------
// エンターキーが押された
//----------------------------------------------------------------------------
void CFlangerCustomizeWnd::OnReturnKeyPressed()
{
	switch(m_nFocus)
	{
		case 0:
...
// スペースキーが押された
//----------------------------------------------------------------------------
void CFlangerCustomizeWnd::OnSpaceKeyPressed()
{
	switch(m_nFocus)
	{
		case 0:
...
// エスケープキーが押された
//----------------------------------------------------------------------------
void CFlangerCustomizeWnd::OnEscKeyPressed()
{
	Close();
}
//----------------------------------------------------------------------------
// 作成された
//----------------------------------------------------------------------------
LRESULT CFlangerCustomizeWnd::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	int nButtonWidth = 90;
	...
	SendMessage(m_waveformEdit, EM_SETLIMITTEXT, 4, 0);
	...
	SendMessage(m_waveformUd, UDM_SETPOS32, 0, (LONG)lWaveform);
	...
	SendMessage(m_delayEdit, EM_SETLIMITTEXT, 5, 0);
	...
	SendMessage(m_delayUd, UDM_SETRANGE32, 0, 4000);
...
// 破棄
//----------------------------------------------------------------------------
void CFlangerCustomizeWnd::OnDestroy()
{
...
// キーボードが押された
//----------------------------------------------------------------------------
void CFlangerCustomizeWnd::OnKeyDown(UINT vk, int cRepeat, UINT flags)
{
	switch(vk)
	{
	case VK_TAB:
...
// 通知が来た
//----------------------------------------------------------------------------
LRESULT CFlangerCustomizeWnd::OnNotify(int idFrom, NMHDR* pnmhdr)
{
