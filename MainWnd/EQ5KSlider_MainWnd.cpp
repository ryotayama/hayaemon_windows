//----------------------------------------------------------------------------
// EQ5KSlider_MainWnd.cpp : イコライザ ( 5KHz ) 設定用スライダの管理を行う
//----------------------------------------------------------------------------
#include <windows.h>
#include "../App.h"
...
// 作成
//----------------------------------------------------------------------------
BOOL CEQ5KSlider_MainWnd::Create()
{
...
// 高さを得る
//----------------------------------------------------------------------------
int CEQ5KSlider_MainWnd::GetHeight() const
{
	if(!m_rMainWnd.GetMenu().IsItemChecked(ID_EQ))
		return m_rMainWnd.GetPanSlider().GetHeight();
	else if(m_rMainWnd.GetMenu().IsItemChecked(ID_EQ5K))
		return CSliderCtrl::GetHeight();
	else return m_rMainWnd.GetEQ4KSlider().GetHeight();
}
//----------------------------------------------------------------------------
// 縦位置を得る
//----------------------------------------------------------------------------
int CEQ5KSlider_MainWnd::GetTop() const
{
	if(!m_rMainWnd.GetMenu().IsItemChecked(ID_EQ))
		return m_rMainWnd.GetPanSlider().GetTop();
	else if(m_rMainWnd.GetMenu().IsItemChecked(ID_EQ5K))
		return CSliderCtrl::GetTop();
	else return m_rMainWnd.GetEQ4KSlider().GetTop();
}
//----------------------------------------------------------------------------
// 位置の再設定
//----------------------------------------------------------------------------
void CEQ5KSlider_MainWnd::ResetPos()
{
	POINT pt;
	pt.x = m_rMainWnd.GetEQ5KLabel().GetLeft();
	pt.y = m_rMainWnd.GetEQ5KLabel().GetTop();
	ScreenToClient(m_rMainWnd, &pt);
	int nLeft = pt.x + m_rMainWnd.GetEQ5KLabel().GetWidth()
...
// サイズの再設定
//----------------------------------------------------------------------------
void CEQ5KSlider_MainWnd::ResetSize()
{
	POINT pt;
	pt.x = GetLeft();
	ScreenToClient(m_rMainWnd, &pt);
...
// コマンド
//----------------------------------------------------------------------------
void CEQ5KSlider_MainWnd::OnCommand(int id, HWND hwndCtl, UINT codeNotify)
{
...
// スクロールされた
//----------------------------------------------------------------------------
void CEQ5KSlider_MainWnd::OnHScroll(HWND hwndCtl, UINT code, int pos)
{
	LONG trackpos = GetThumbPos();
	m_rMainWnd.SetEQ5K((int)trackpos);
	m_rMainWnd.GetEQ5KLabel().SetEQ5K(trackpos);
}
//----------------------------------------------------------------------------
// キーボードが押された
//----------------------------------------------------------------------------
void CEQ5KSlider_MainWnd::OnKeyDown(UINT vk, int cRepeat, UINT flags)
{
	if(vk == VK_TAB) {
		...
		SetThumbPos(0);
		m_rMainWnd.SetEQ5K(0);
		m_rMainWnd.GetEQ5KLabel().SetEQ5K(0);
		return;
	}
...
// クリックされた
//----------------------------------------------------------------------------
void CEQ5KSlider_MainWnd::OnLButtonDown(int x, int y, UINT keyFlags)
{
	RECT rc;
...
// マウスホイールがスクロールされた
//----------------------------------------------------------------------------
BOOL CEQ5KSlider_MainWnd::OnMouseWheel(UINT nFlags, int zDelta, POINTS pt)
{
	tstring strEQ = m_rMainWnd.GetEQ5KLabel().GetEdit().GetText();
	...
	if(n < nMin) n = nMin;
	if(n > nMax) n = nMax;
	m_rMainWnd.SetEQ5K(n);
	m_rMainWnd.GetEQ5KLabel().SetEQ5K(n);
	return FALSE;
}
//----------------------------------------------------------------------------
// 右クリックが離された
//----------------------------------------------------------------------------
void CEQ5KSlider_MainWnd::OnRButtonUp(int x, int y, UINT keyFlags)
{
	m_rClickMenu.Create();
	POINT pt;
	GetCursorPos(&pt);
