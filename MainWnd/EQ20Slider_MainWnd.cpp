//----------------------------------------------------------------------------
// EQ20Slider_MainWnd.cpp : イコライザ ( 20Hz ) 設定用スライダの管理を行う
//----------------------------------------------------------------------------
#include <windows.h>
#include "../App.h"
...
// 作成
//----------------------------------------------------------------------------
BOOL CEQ20Slider_MainWnd::Create()
{
...
// 高さを得る
//----------------------------------------------------------------------------
int CEQ20Slider_MainWnd::GetHeight() const
{
...
// 縦位置を得る
//----------------------------------------------------------------------------
int CEQ20Slider_MainWnd::GetTop() const
{
	if(!m_rMainWnd.GetMenu().IsItemChecked(ID_EQ))
...
// 位置の再設定
//----------------------------------------------------------------------------
void CEQ20Slider_MainWnd::ResetPos()
{
	POINT pt;
	pt.x = m_rMainWnd.GetEQ20Label().GetLeft();
	pt.y = m_rMainWnd.GetEQ20Label().GetTop();
	ScreenToClient(m_rMainWnd, &pt);
	int nLeft = pt.x + m_rMainWnd.GetEQ20Label().GetWidth()
...
// サイズの再設定
//----------------------------------------------------------------------------
void CEQ20Slider_MainWnd::ResetSize()
{
	POINT pt;
	pt.x = GetLeft();
	ScreenToClient(m_rMainWnd, &pt);
...
// コマンド
//----------------------------------------------------------------------------
void CEQ20Slider_MainWnd::OnCommand(int id, HWND hwndCtl, UINT codeNotify)
{
...
// スクロールされた
//----------------------------------------------------------------------------
void CEQ20Slider_MainWnd::OnHScroll(HWND hwndCtl, UINT code, int pos)
{
	LONG trackpos = GetThumbPos();
	m_rMainWnd.SetEQ20((int)trackpos);
	m_rMainWnd.GetEQ20Label().SetEQ20(trackpos);
}
//----------------------------------------------------------------------------
// キーボードが押された
//----------------------------------------------------------------------------
void CEQ20Slider_MainWnd::OnKeyDown(UINT vk, int cRepeat, UINT flags)
{
	if(vk == VK_TAB) {
		...
		SetThumbPos(0);
		m_rMainWnd.SetEQ20(0);
		m_rMainWnd.GetEQ20Label().SetEQ20(0);
		return;
	}
...
// クリックされた
//----------------------------------------------------------------------------
void CEQ20Slider_MainWnd::OnLButtonDown(int x, int y, UINT keyFlags)
{
	RECT rc;
...
// マウスホイールがスクロールされた
//----------------------------------------------------------------------------
BOOL CEQ20Slider_MainWnd::OnMouseWheel(UINT nFlags, int zDelta, POINTS pt)
{
	tstring strEQ = m_rMainWnd.GetEQ20Label().GetEdit().GetText();
	...
	m_rMainWnd.GetEQ20Label().SetEQ20(n);
	return FALSE;
}
//----------------------------------------------------------------------------
// 右クリックが離された
//----------------------------------------------------------------------------
void CEQ20Slider_MainWnd::OnRButtonUp(int x, int y, UINT keyFlags)
{
	m_rClickMenu.Create();
	POINT pt;
	GetCursorPos(&pt);
